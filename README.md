...
| Stage | What I Tried & Why | Evidence (Fill in post-eval) | Decision / Learning |
| :--- | :--- | :--- | :--- |
| **1. Zero-Shot Baseline** | Sent raw ticket subject and body to Claude with a generic prompt (`"You are a support agent..."`) to establish an unassisted baseline. | *[Placeholder: Baseline Factual Accuracy: ___%, Hallucinations: ___%]* | Baseline hallucinated non-existent refund policies (e.g. 30-day refund for monthly users) and had zero awareness of customer tier limits. |
| **2. TF-IDF Documentation Retrieval** | Implemented heading-aware chunking (~200 words) and local NumPy TF-IDF search to ground responses without external embedding API latency. | *[Placeholder: Retrieval Top-3 Hit Rate: ___% on FAQ tickets]* | Exact technical keyword matching (`ERR_WS_DISCONNECTED_502`, `14-day`, `VAT`) performed with zero network latency and perfect precision. |
| **3. Customer Context Grounding** | Integrated `get_customer_context` tool to dynamically inject plan tier (Free/Pro/Team), signup date, and past ticket volume. | *[Placeholder: Tier Awareness Accuracy: ___%]* | Agent stopped suggesting Pro features to Free users without explaining upgrade steps, and applied enterprise SLAs accurately. |
| **4. Two-Pass Verification & Auto-Rewrite** | Built a secondary QA inspector LLM call in `verify.py` that cross-examines draft sentences against retrieved docs and triggers a targeted rewrite if claims are ungrounded. | *[Placeholder: Post-Verification Hallucination Reduction: -___%]* | Successfully caught edge cases (e.g. contradictory VR headset tickets) and corrected unverified assertions before saving. |
| **5. Human-in-the-Loop Review Queue** | Replaced all direct send actions with an append-only JSON review queue (`/agent/review_queue`) and built the `approve_reply.py` interactive CLI. | *[Placeholder: Human Approval Rate: ___%, Rejection Rate: ___%]* | Guarantees 100% human oversight before customer delivery, satisfying strict security and compliance requirements. |
