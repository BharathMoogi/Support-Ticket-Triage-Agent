"""
server.py — Lightweight Localhost Web Dashboard & Review UI.

Provides an interactive browser interface at http://localhost:8000 to:
1. Browse and review pending drafts (Approve, Edit, Reject).
2. Inspect per-ticket agent trajectories & reasoning traces.
3. View the knowledge base documentation articles.
4. View Baseline vs Agent evaluation comparison and metrics.

Zero extra dependencies — runs purely with Python standard library http.server.
"""
    ...
    <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
      ...
      <div class="flex items-center space-x-4">
        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-emerald-950 text-emerald-400 border border-emerald-800">
          <span class="w-2 h-2 rounded-full bg-emerald-400 mr-2 animate-pulse"></span> Localhost Server Active
        </span>
      </div>
    </div>
  </header>
      ...
      if (!items.length) {
        container.innerHTML = `
          <div class="bg-slate-800/60 border border-slate-700 rounded-xl p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-white">Review Queue is Empty</h3>
            <p class="text-sm text-slate-400 mt-1">Run <code class="text-indigo-400 font-mono bg-slate-900 px-2 py-0.5 rounded">python agent/main.py</code> to generate verified drafts.</p>
          </div>`;
        return;
      }
...
class DashboardRequestHandler(BaseHTTPRequestHandler):
    def log_message(self, format: str, *args: Any) -> None:
        # Silence standard request logs for a cleaner terminal
        pass
    ...
    def do_GET(self) -> None:
        ...
        if path == "/" or path == "/index.html":
            ...
            from agent.review_queue import list_review_queue
            items = list_review_queue(BASE_DIR / "agent" / "review_queue")
